...
from timeseriesgym.data import is_dataset_prepared
from timeseriesgym.registry import Competition, registry
from timeseriesgym.utils import create_run_dir, get_logger, get_runs_dir, get_timestamp
...
async def worker(
    ...
    while True:
        ...
        run_logger.info(
            f"[Worker {idx}] Running seed {task.seed} for {task.competition.id} and agent "
            f"{task.agent.name}"
        )
        ...
        try:
            ...
            run_logger.info(
                f"[Worker {idx}] Finished running seed {task.seed} for {task.competition.id} and "
...
async def main(args):
    ...
    if agent.privileged and os.environ.get(
        ...
        raise ValueError(
            ...
            "`I_ACCEPT_RUNNING_PRIVILEGED_CONTAINERS` is not set to `True`! Carefully consider if "
            "you wish to run this agent before continuing. See agents/README.md for more details."
        )

    run_group = f"{get_timestamp()}_run-group_{agent.name}"

    # Load competition ids and check all are prepared
    with open(args.competition_set) as f:
    ...
    with open(args.container_config) as f:
    ...
    run_group_dir = get_runs_dir(args.run_dir) / run_group
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--container-config",
        help="Path to a JSON file with an environment configuration; these args will be passed to "
        "`docker.from_env().containers.create`",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--start-seed",
        help="Seed to start from. If not specified, will start from 0.",
        type=int,
        required=False,
        default=0,
    )
    parser.add_argument(
        ...
        help="Whether to allow empty private directories.",
        action="store_true",
        required=False,
        default=False,
    )
    parser.add_argument(
        ...
        help="Whether to allow empty answers files.",
        action="store_true",
        required=False,
        default=False,
    )
