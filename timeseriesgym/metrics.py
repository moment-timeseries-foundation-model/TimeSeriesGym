...
from timeseriesgym import metrics_helpers
from timeseriesgym.metrics_helpers import ParticipantVisibleError, adjust_predictions
...
def symmetric_mean_absolute_percentage_error(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    """
    return np.mean(200 * np.abs(y_pred - y_true) / (np.abs(y_true + 1) + np.abs(y_pred + 1)))
...
def kl_divergence(
    ...
    for col in solution.columns:
        ...
        # Clipping the max avoids cases where the loss would be infinite or undefined, clipping
        # the min prevents users from playing games with the 20th decimal place of predictions.
...
def kl_divergence_score(
    ...
    The predictions in each row must add to one so any probability assigned to a case where y == 0
    reduces another prediction where y > 0, so crucially there is an important indirect effect.

    https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence

    solution: pd.DataFrame
    submission: pd.DataFrame
    epsilon: KL divergence is undefined for p=0 or p=1. If epsilon is not null, solution and
    submission probabilities are clipped to max(eps, min(1 - eps, p).
    row_id_column_name: str
    ...
    >>> score(
            pd.DataFrame({'id': range(4), 'ham': [0, 1, 1, 0],
                          'spam': [1, 0, 0, 1]}),
            pd.DataFrame({'id': range(4), 'ham': [.1, .9, .8, .35],
                          'spam': [.9, .1, .2, .65]}), row_id_column_name=row_id_column_name)
    0.216161...
    >>> solution = pd.DataFrame({'id': range(3), 'ham': [0, 0.5, 0.5], 'spam': [0.1, 0.5, 0.5],
                                 'other': [0.9, 0, 0]})
    >>> submission = pd.DataFrame({'id': range(3), 'ham': [0, 0.5, 0.5], 'spam': [0.1, 0.5, 0.5],
                                   'other': [0.9, 0, 0]})
    >>> score(solution, submission, 'id')
    0.0
    >>> solution = pd.DataFrame({'id': range(3), 'ham': [0, 0.5, 0.5], 'spam': [0.1, 0.5, 0.5],
                                 'other': [0.9, 0, 0]})
    >>> submission = pd.DataFrame({'id': range(3), 'ham': [0.2, 0.3, 0.5], 'spam': [0.1, 0.5, 0.5],
                                   'other': [0.7, 0.2, 0]})
    >>> score(solution, submission, 'id')
    0.160531...
    """
    ...
    metrics_helpers.verify_valid_probabilities(solution, "solution")
    metrics_helpers.verify_valid_probabilities(submission, "submission")

    return metrics_helpers.safe_call_score(
        kl_divergence,
        solution,
        submission,
        epsilon=epsilon,
